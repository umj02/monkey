...
import { deleteCalendarEventRemote, fetchCalendarEvents, upsertCalendarEvent } from "@/lib/services/supabase-data-service";
...
export function useCalendarEvents() {
  ...
  useEffect(() => {
    ...
    setSyncing(true);
    fetchCalendarEvents().then((remote) => {
  ...
  function updateEvent(id: string, input: CalendarEventInput) {
    ...
    setEvents((list) => sortCalendarEvents(list.map((item) => item.id === id ? event : item)));
    if (session && mode === "supabase") void upsertCalendarEvent(event);
  }
  ...
  return { events, setEvents, ready, syncing, createEvent, updateEvent, deleteEvent };
}
