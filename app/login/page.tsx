...
export default function LoginPage() {
  ...
  const { loginWithProfile, loginWithSocial } = useAuth();
  ...
  function submit() {
    ...
    loginWithProfile({ name: profile.name, email: email.trim() });
    router.push("/today");
  }
  ...
  return (
    <main className="app-screen px-6 py-10">
      ...
      <section className="mx-auto max-w-[360px] text-center">
        ...
        <p className="mt-1 text-sm text-monkey-muted">Login mock listo para Supabase Auth</p>
        <form className="mt-7 space-y-3 text-left" onSubmit={(event) => { event.preventDefault(); submit(); }}>
          ...
          <div className="text-right"><button type="button" onClick={() => setToast({ message: "Luego lo conectamos con Supabase Auth", type: "success" })} className="text-xs font-bold text-monkey-green">Forgot password?</button></div>
          <button type="submit" className="flex h-14 w-full items-center justify-center rounded-pill bg-monkey-green text-sm font-bold text-white shadow-float transition active:scale-95">Login</button>
        </form>
