...
import { getPlannedExpenseStatus, getWalletPeriodRange, isPlannedExpenseInPeriod, isTransactionInPeriod, PLANNED_EXPENSE_CATEGORIES, VARIABLE_EXPENSE_CATEGORIES } from "@/lib/services/wallet-service";
...
const frequencyLabels = { monthly: "Mensual", biweekly: "Quincenal", weekly: "Semanal", yearly: "Anual", one_time: "Una vez" } as const;
const statusMap = {
...
export default function WalletPage() {
  ...
  const plannedInPeriod = useMemo(() => wallet.plannedExpenses.filter((expense) => expense.enabled !== false && isPlannedExpenseInPeriod(expense, wallet.period)).sort((a, b) => a.dueDate.localeCompare(b.dueDate)), [wallet.plannedExpenses, wallet.period]);
  const plannedTotals = plannedInPeriod.reduce((acc, expense) => { const status = getPlannedExpenseStatus(expense); acc.total += expense.amount; if (status === "paid") acc.paid += expense.amount; else if (status === "overdue") acc.overdue += expense.amount; else acc.pending += expense.amount; return acc; }, { total: 0, paid: 0, pending: 0, overdue: 0 });
  ...
  function changeType(nextType: WalletTransactionType) { setType(nextType); setCategory(defaultCategory(nextType)); setIcon(defaultIcon(nextType)); }
